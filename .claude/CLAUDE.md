...
### 설계 결정사항
- [x] TCP 방식 선택 (DataInputStream/DataOutputStream 사용)
- [x] 게임 상태 동기화 주기: 20 Hz (초당 20번)
- [x] 네트워크 지연 처리: 서버 권한 모델 (클라이언트 예측은 선택사항)
- [x] 플레이어 연결 끊김 처리: 기본적인 감지 및 알림
- [x] 게임 로비 시스템: 간단한 대기실 구현
...
- [x] 네트워크 통신 프로토콜 설계
- [x] 클라이언트-서버 아키텍처 설계
- [ ] UML 다이어그램 작성 (시퀀스, 클래스) - 시간 부족 시 생략
...
- **네트워크 방식**: TCP 소켓 (DataInputStream/DataOutputStream)
- **플레이어 수**: 2명 (호스트 + 게스트)
- **게임 모드**: 협력 모드 (Co-op)
- **동기화 주기**: 20 Hz (초당 20번)
- **메시지 포맷**: 수업 자료 기반 DataStream 패턴
...
- **완료**:
  ...
  - 프로젝트 설정 구성
  - 수업 자료 요약 정리 (LECTURE_NOTES.md)
  - 기존 게임 아키텍처 분석 완료
    - GameEngine: 60 FPS 게임 루프
    - Mario: 플레이어 캐릭터 (위치, 생명, 점수)
    - MapManager: 맵 및 오브젝트 관리
    - UIManager: Swing 기반 렌더링
  - 네트워크 아키텍처 설계 완료 (GAME_DESIGN.md)
    - 게임 모드: 협력 모드 (2인 Co-op) 결정
    - 네트워크 방식: TCP + DataStream 결정
    - 프로토콜 설계: 메시지 타입 및 게임 상태 패킷 정의
    - 구현 우선순위 설정

- **기술 결정**:
  - TCP 소켓 사용 (수업 자료 패턴 기반)
  - DataInputStream/DataOutputStream 활용
  - 서버-클라이언트 모델 (서버가 게임 로직 권한)
  - 20 Hz 네트워크 업데이트 주기
...
**마지막 업데이트**: 2024-12-14 오후
**상태**: Phase 1 완료 → Phase 2 시작 준비
**다음 마일스톤**: 기본 네트워크 연결 구현
